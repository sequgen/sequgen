...
def test_with_fttf_and_location():
    ...
    with pytest.raises(AssertionError) as excinfo:
        triangular_peak(t_predict, **opts, location=1.0, unit_integral=False)
    assert "Underspecified curve" in str(excinfo.value)
...
def test_with_fttt_and_location():
    ...
    opts = dict(width=None,
                ...
                width_base_right=1.0,
                height=1.0)
    actual = triangular_peak(t_predict, **opts, location=1.0, unit_integral=False)
...
def test_with_fttf_with_skewness_and_location():
    ...
    opts = dict(width=None,
                ...
                height=None)
    with pytest.raises(AssertionError) as excinfo:
        triangular_peak(t_predict, **opts, location=location, unit_integral=False)
    assert "Underspecified curve" in str(excinfo.value)
...
def test_with_fttf_small_base_right_irregular_sampling():
    ...
    opts = dict(width=None,
                ...
                height=None)
    with pytest.raises(AssertionError) as excinfo:
        triangular_peak(t_predict, **opts, location=3.0, unit_integral=False)
    assert "Underspecified curve" in str(excinfo.value)
